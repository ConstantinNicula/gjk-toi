...
class CollisionMesh: 
    ...
    def get_transformed_verts(self, R: glm.mat3, s: glm.vec3, t: glm.vec3) -> list[glm.vec3]:
        return list(map(lambda v: R * (s * v) + t, self.verts))


def debug_test():
    mesh_verts = glm_utils.to_glm_vec_list([[1, 1], [2, 1], [2, 2], [1, 2], [0.5, 1.5]])
    mesh = CollisionMesh(mesh_verts)
    world_verts = mesh.get_transformed_verts( glm_utils.rotation_from_euler(0, 0, 90), glm.vec3(1, 1, 1), glm.vec3(0, 0, 1))
    print(world_verts) 
